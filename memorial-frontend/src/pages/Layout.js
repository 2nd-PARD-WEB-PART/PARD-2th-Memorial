    ...
    return (
        <div>
            <Div justifyContent="space-between" height="40px" bgColor="black">
                ...
                <Link to={
                    localStorage.getItem('emailVerified') === 'true' ? "/HomePage" : "/"
                } style={linkStyle} activeStyle={hoverStyle}>
                    <Menu>천국 말고 홈화면으로 돌아가시기</Menu>
                </Link>
                </Div>
                <Div height="40px" justifyContent="end">
                    <Link to={localStorage.getItem('emailVerified') === 'true' ? "/AddPage" : "/"} style={linkStyle}>
                        <Menu>장례식 등록하기</Menu>
                    </Link>
                    <Link to={localStorage.getItem('emailVerified') === 'true' ? "/MyPage" : "/"} style={linkStyle}>
                        <Menu>마이페이지</Menu>
                    </Link>
